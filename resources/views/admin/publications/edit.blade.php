...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-gray-600">Update publication or economic activity document</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form action="{{ route('admin.publications.update', $publication) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="type" 
                    ...
                    <option value="publication" {{ old('type', $publication->type) === 'publication' ? 'selected' : '' }}>प्रकाशनहरू (Publications)</option>
                    <option value="economic_activity" {{ old('type', $publication->type) === 'economic_activity' ? 'selected' : '' }}>आर्थिक गतिविधि (Economic Activities)</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            @if($publication->document_file)
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-900">{{ $publication->document_name }}</p>
                            <p class="text-xs text-gray-500">{{ $publication->document_size_formatted }} • {{ strtoupper($publication->document_type) }}</p>
                        </div>
                        <a href="{{ route('admin.publications.download', $publication) }}" 
                           class="ml-auto inline-flex items-center px-3 py-1 bg-[#0073b7] text-white text-xs rounded hover:bg-[#004a7f] transition-colors">
            ...
            <div>
                <label for="document" class="block text-sm font-medium text-gray-700 mb-2">
                    Document {{ $publication->document_file ? '(Replace current document)' : '' }}
                </label>
                ...
                <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-gray-400 transition-colors cursor-pointer" onclick="document.getElementById('document').click()">
                    <div class="space-y-1 text-center" id="upload-display">
                        ...
                        @if($publication->document_file)
                            <p class="text-xs text-gray-400">Leave empty to keep current document</p>
                        @endif
                    </div>
                </div>
                @error('document')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="datetime-local" 
                           id="published_at" 
                           name="published_at" 
                           value="{{ old('published_at', $publication->published_at ? $publication->published_at->format('Y-m-d\TH:i') : '') }}"

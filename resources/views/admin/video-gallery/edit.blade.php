...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600">Update video information</p>
        </div>
        ...
        <form action="{{ route('admin.video-gallery.update', $videoGallery) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $videoGallery->title) }}" required
                ...
                <div>
                    ...
                    <select id="category_id" name="category_id"
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $videoGallery->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <input type="text" id="youtube_embed_code" name="youtube_embed_code" value="{{ old('youtube_embed_code', $videoGallery->youtube_embed_code) }}" required
                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0073b7] focus:border-transparent"
            ...
            <div>
                ...
                <input type="text" id="duration" name="duration" value="{{ old('duration', $videoGallery->duration) }}"
                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0073b7] focus:border-transparent"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0073b7] focus:border-transparent"
                          placeholder="Optional description for this video">{{ old('description', $videoGallery->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $videoGallery->sort_order) }}" min="0"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#0073b7] focus:border-transparent">
                    @error('sort_order')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror

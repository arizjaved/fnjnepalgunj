...
@section('title', 'Create Media Category - Admin Panel')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Create Media Category</h1>
            <p class="text-gray-600">Add a new category for organizing photos and videos</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.media-categories.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="type" name="type" required
                        ...
                        <option value="both" {{ old('type') == 'both' ? 'selected' : '' }}>Both Photo & Video</option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror

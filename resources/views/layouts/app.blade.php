                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center py-2">
                    ...
                    <div class="flex items-center space-x-6">
                        ...
                        <div class="hidden md:flex items-center space-x-4">
                            @if($footerSettings['footer_contact_phone'] ?? null)
                                <a href="tel:{{ str_replace([' ', '-'], '', $footerSettings['footer_contact_phone']) }}" class="flex items-center text-slate-600 hover:text-[#0073b7] transition-colors duration-200">
                                    ...
                                    <span class="text-xs font-medium">{{ $footerSettings['footer_contact_phone'] }}</span>
                                </a>
                            @endif
                            @if($footerSettings['footer_contact_email'] ?? null)
                                <a href="mailto:{{ $footerSettings['footer_contact_email'] }}" class="flex items-center text-slate-600 hover:text-[#0073b7] transition-colors duration-200">
                                    ...
                                    <span class="text-xs font-medium">{{ $footerSettings['footer_contact_email'] }}</span>
                                </a>
                            @endif
                        </div>
                    </div>
                        ...
                        <a href="{{ route('home') }}" class="flex items-center group">
                            <div class="relative">
                                ...
                                @if(site_setting('site_logo'))
                                    <img src="{{ asset('storage/' . site_setting('site_logo')) }}" alt="{{ site_setting('site_name', 'FNJ') }} Logo" class="relative h-20 w-20 mr-6 rounded-xl shadow-2xl border-2 border-white/20 group-hover:scale-105 transition-transform duration-300 object-cover"/>
                                @else
                            ...
                            <div class="space-y-1">
                                <h1 class="text-4xl lg:text-5xl font-bold text-white drop-shadow-lg tracking-tight">
                                    {{ site_setting('site_name', 'नेपाल पत्रकार महासंघ') }}
                                </h1>
                                <p class="text-lg text-white/80 font-medium tracking-wide">
                                    {{ site_setting('site_name_english', 'Federation of Nepali Journalists') }}
                                </p>
                                <div class="flex items-center space-x-4 mt-2">
                                    @if(site_setting('organization_location'))
                                        <div class="flex items-center text-white/70 text-sm">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                            </svg>
                                            {{ site_setting('organization_location', 'काठमाडौं, नेपाल') }}
                                        </div>
                                    @endif
                                    @if(site_setting('organization_establishment'))
                                        <div class="flex items-center text-white/70 text-sm">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                            {{ site_setting('organization_establishment', 'स्थापना: २०४६ साल') }}
                                        </div>
                                    @endif
                                </div>
                            </div>
        ...
        <footer class="bg-gradient-to-r from-[#004a7f] via-[#0073b7] to-[#004a7f] text-gray-200">
            <div class="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div class="md:col-span-1">
                        <h3 class="text-white text-lg font-bold mb-4 border-b border-[#0073b7] pb-2">FOLLOW US ON</h3>
                        <div class="flex space-x-3 mb-6">
                            @if($socialSettings['facebook_url'] ?? null)
                                <a href="{{ $socialSettings['facebook_url'] }}" target="_blank" class="bg-white/10 hover:bg-white/20 text-white rounded-full p-3 w-12 h-12 flex items-center justify-center transition-all duration-200 hover:scale-110">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fill-rule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clip-rule="evenodd" /></svg>
                                </a>
                            @endif
                            @if($socialSettings['twitter_url'] ?? null)
                                <a href="{{ $socialSettings['twitter_url'] }}" target="_blank" class="bg-white/10 hover:bg-white/20 text-white rounded-full p-3 w-12 h-12 flex items-center justify-center transition-all duration-200 hover:scale-110">
                            ...
                            @if($socialSettings['youtube_url'] ?? null)
                                <a href="{{ $socialSettings['youtube_url'] }}" target="_blank" class="bg-white/10 hover:bg-white/20 text-white rounded-full p-3 w-12 h-12 flex items-center justify-center transition-all duration-200 hover:scale-110">
                            ...
                            @endif
                            @if($socialSettings['instagram_url'] ?? null)
                                <a href="{{ $socialSettings['instagram_url'] }}" target="_blank" class="bg-white/10 hover:bg-white/20 text-white rounded-full p-3 w-12 h-12 flex items-center justify-center transition-all duration-200 hover:scale-110">
                        ...
                        <!-- Newsletter Section -->
                        @if($isNewsletterEnabled && ($footerSettings['footer_newsletter_enabled'] ?? '1') === '1')
                            <div class="border-t border-[#0073b7]/30 pt-4">
                                <h4 class="text-white text-base font-semibold mb-3">{{ $footerSettings['footer_newsletter_title'] ?? 'समाचार सदस्यता' }}</h4>
                                <p class="text-gray-300 text-xs mb-3">{{ $footerSettings['footer_newsletter_description'] ?? 'नवीनतम समाचार र अपडेटहरू प्राप्त गर्न सदस्यता लिनुहोस्' }}</p>
                                <form class="flex gap-2">
                                    <input type="email" placeholder="इमेल ठेगाना" class="flex-1 px-3 py-2 text-sm rounded-lg bg-white/10 border border-white/20 text-white placeholder-gray-300 focus:outline-none focus:ring-2 focus:ring-[#0073b7] focus:border-transparent">
                                    <button type="submit" class="bg-[#0073b7] hover:bg-[#004a7f] text-white px-4 py-2 text-sm rounded-lg transition-colors font-medium">
                                        सदस्यता
                                    </button>
                                </form>
                            </div>
                    ...
                    <div class="md:col-span-1">
                        ...
                        <ul class="space-y-3 text-sm">
                            @if($footerSettings['footer_contact_address'] ?? null)
                        ...
                        <!-- Powered By Section -->
                        <div class="mt-6 pt-4 border-t border-[#0073b7]/30">
                            <div class="flex items-center justify-center bg-white/10 rounded-lg p-3 hover:bg-white/20 transition-colors">
                                <div class="text-center">
                                    <p class="text-xs text-gray-300 mb-2">Powered By</p>
                                    <div class="bg-white rounded px-3 py-1">
                                        <img src="{{ asset('images/dotworks-logo.svg') }}" alt="Dotworks" class="h-6 w-auto mx-auto" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                                        <span class="text-[#0073b7] font-bold text-sm hidden">Dotworks</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
                    <div class="flex flex-col md:flex-row justify-between items-center text-center space-y-2 md:space-y-0">
                        <p class="flex items-center">
                            ...
                            {{ $copyrightText ?: 'प्रतिलिपी अधिकार © ' . date('Y') . ' नेपाल पत्रकार महासंघ। सबै अधिकार सुरक्षित।' }}
                        </p>
                        <div class="flex items-center space-x-4">
                            @if($footerMenuLinks && count($footerMenuLinks) > 0)
                                @foreach($footerMenuLinks as $link)
                                    <a href="{{ $link['url'] ?? '#' }}" class="hover:text-[#0073b7] transition-colors">{{ $link['name'] ?? '' }}</a>
                            ...
                            @endauth
                            <span>|</span>
                            <p>Designed and developed by <a href="#" class="text-[#0073b7] hover:text-white hover:underline transition-colors">Dotworks Technologies</a></p>
                        </div>
                    </div>
                </div>
            </div>
